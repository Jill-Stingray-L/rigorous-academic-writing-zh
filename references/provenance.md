...
- 用户提供的“下一轮完善指令”（2026-08-26）。
- 用户提供的“应用型研究成果可迁移性”增量改进简报（2026-08-27）。

这些维护简报推动了成果最低构件、范围—案例—结论、汇总—明细追溯、输入标签预审、First-use Audit、章节删除测试、图形抽象层级、图表信息增量、视觉语义、反馈分类、依赖传播，以及应用型成果可迁移性和方法论交付闭环等行为约束。运行规范按单一主要定义位置组织；简报中的案例不转化为固定词典或机械阈值。
...
## 九、最终知识状态与防御性表达（2026-09-05）

- 用户提供的“系统性审计和结构性强化”要求及其中的真实失败模式。
- `tests/fixtures/forward_review_report.md`：结果段把湿度分析写成待应用方法，第 3—5 章混入“计划、取得记录后、尚未执行”等工作状态；表 1 完整重复正文数字且没有解释或下游用途。
- `tests/fixtures/feedback_sources.docx`：同一页同时包含评审批注、修订删除与新增、黄色高亮、红色文字、必要讨论边界和“后续拟补充”工作记录，支持区分反馈来源、当前稿优先级、真实限制和作者待办。

这些材料暴露的核心问题不是个别禁词，而是章节知识层级、正文状态、限制安置和版本权威关系未先被裁决。由此新增：六类章节知识层级、防御性说明三分法、证据边界按影响范围集中安置、枚举覆盖与概念层级、文献“主张—来源—采用方式”、完整报告自足性，以及用户当前稿优先于旧稿和控制底稿。

脚本只为已出现且可保守定位的现象增加人工复核线索：工作日志式未来任务、理论章节中的项目执行语言、扩展防御性澄清、节级防御密度和同节重复证据边界。枚举成员关系、文献采用方式、图表信息增量和版本取舍仍依赖语义与材料，未转化为正则裁决、自动删除或项目专属词典。
